    ...
    async def get_all_detections(
        ...
        async with http(trust_env=True) as session:
            async with session.get(
                ...
                if response.status == 200:
                    print(
                        f'{datetime.now().strftime("%X.%f")[:-3]} | STK | get detections'
                    )
                    return loads(str(await response.text()))
                else:  # Exceptions
                    raise Exception(
                        f'{datetime.now().strftime("%X.%f")[:-3]} | STK | get_detections{response.status}:\n'
                        f"    | creds: {self.login}:{self.passw}\n"
...
class CMDLineArguments(argparse.ArgumentParser):
    ...
    def __init__(self) -> None:
        ...
        self.add_argument(
            "-b", "--test-bot", help="start with TEST_TOKEN env", action="store_true"
        )
